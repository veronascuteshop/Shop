...
(function () {
  ...
  function subirImagen(ruta, dataUrl, cb) {
    var c = cfg();
    if (!isReady()) return cb("Sin conexión con GitHub");
    ...
    fetch(url + "?ref=" + encodeURIComponent(branch), { headers: headers(c) })
      ...
      .then(function (actual) {
        // si ya está subida y no cambió, no la volvemos a subir
        if (actual && actual.size && Math.abs(actual.size - Math.floor(base64.length * 0.75)) < 32) {
          return { yaEstaba: true };
        }
        var payload = { message: "Sube una imagen de la tienda", content: base64, branch: branch };
        if (actual && actual.sha) payload.sha = actual.sha;
        return fetch(url, { method: "PUT", headers: headers(c), body: JSON.stringify(payload) })
          .then(function (r) {
            return r.json().then(function (b) {
              if (!r.ok) throw explain(r.status, b);
              return b;
            });
          });
      })
      .then(function () { cb(null, ruta); })
      .catch(function (e) { cb(typeof e === "string" ? e : "No se pudo subir la imagen"); });
  }
