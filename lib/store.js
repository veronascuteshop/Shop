...
(function () {
  ...
  function readImage(file, maxSide, quality, cb) {
    ...
    function procesar(fuente, w, h) {
      ...
      } catch (e) { return done("No se pudo procesar la imagen. Prueba con otra."); }
  ...
  function quitarFondo(dataUrl, tolerancia, cb) {
    ...
    img.onload = function () {
      ...
      var fin = 0;
      var y, x;
      for (x = 0; x < w; x++) { cola[fin++] = x; cola[fin++] = (h - 1) * w + x; }
      for (y = 0; y < h; y++) { cola[fin++] = y * w; cola[fin++] = y * w + (w - 1); }
      ...
      while (ini < fin) {
        ...
        visto[p] = 1;
        var i = p * 4;
        if (lejos(color(i), fondo) > tol) continue;
        px[i + 3] = 0;                                   // transparente
        var py = (p / w) | 0, pxx = p % w;
        if (pxx > 0 && !visto[p - 1] && fin < cola.length) cola[fin++] = p - 1;
        if (pxx < w - 1 && !visto[p + 1] && fin < cola.length) cola[fin++] = p + 1;
        if (py > 0 && !visto[p - w] && fin < cola.length) cola[fin++] = p - w;
        if (py < h - 1 && !visto[p + w] && fin < cola.length) cola[fin++] = p + w;
      }
